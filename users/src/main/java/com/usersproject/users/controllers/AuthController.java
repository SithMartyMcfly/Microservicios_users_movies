...
public class AuthController {
    ...
    public User getLoginUser (User usuario){
        ...
        List<User> lista = entityManager.createQuery("From User u where u.email = :email", User.class)
            ...
            if (lista.isEmpty()){
                return null;
            }//HASTA AQUI VERIFICACIÓN DEL NOMBRE O MAIL DEL USUARIO
    ...
    public ResponseEntity<?> login (@RequestBody User usuario) {
        ...
        //Nos ha devuelto un usuario getLoginUser() porque el usuario es correcto
        if(loggedUser != null){
            ...
            String tokenJWT = jwtUtil.create(
                                            String.valueOf(loggedUser.getId()), // hay que pasar a String el id
                                            loggedUser.getEmail()
                                            );
